package com.mms.controller.system.head;
...
import com.mms.service.system.fhsms.FhsmsManager;
import com.mms.service.system.user.UserManager;
import com.mms.service.system.userphoto.UserPhotoManager;
import com.mms.util.AppUtil;
import com.mms.util.Const;
import com.mms.util.Jurisdiction;
import com.mms.util.PageData;
import com.mms.util.SmsUtil;
import com.mms.util.Tools;
import com.mms.util.Watermark;
import com.mms.util.mail.SimpleMailSender;
...
public class HeadController extends BaseController {
	...
	public ModelAndView saveSys() throws Exception{
		...
		Tools.writeFile(Const.EMAIL,pd.getString("SMTP")+",mms,"+pd.getString("PORT")+",mms,"+pd.getString("EMAIL")+",mms,"+pd.getString("PAW"));	//写入邮件服务器配置
		Tools.writeFile(Const.SMS1,pd.getString("SMSU1")+",mms,"+pd.getString("SMSPAW1"));	//写入短信1配置
		Tools.writeFile(Const.SMS2,pd.getString("SMSU2")+",mms,"+pd.getString("SMSPAW2"));	//写入短信2配置
		mv.addObject("msg","OK");
	...
	public ModelAndView saveSys2() throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		Tools.writeFile(Const.FWATERM,pd.getString("isCheck1")+",mms,"+pd.getString("fcontent")+",mms,"+pd.getString("fontSize")+",mms,"+pd.getString("fontX")+",mms,"+pd.getString("fontY"));	//文字水印配置
		Tools.writeFile(Const.IWATERM,pd.getString("isCheck2")+",mms,"+pd.getString("imgUrl")+",mms,"+pd.getString("imgX")+",mms,"+pd.getString("imgY"));	//图片水印配置
		Watermark.fushValue();
		mv.addObject("msg","OK");
		mv.setViewName("save_result");
		return mv;
	...
	public ModelAndView saveSys3() throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		Tools.writeFile(Const.WEBSOCKET,pd.getString("WIMIP")+",mms,"+pd.getString("WIMPORT")+",mms,"+pd.getString("OLIP")+",mms,"+pd.getString("OLPORT")+",mms,"+pd.getString("smsSound"));	//websocket配置
		mv.addObject("msg","OK");
		mv.setViewName("save_result");
		return mv;
	}
	...
	public ModelAndView saveSys4() throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		Tools.writeFile(Const.LOGINEDIT,pd.getString("isZhuce")+",mms,"+pd.getString("isMusic"));	//登录页面配置
		mv.addObject("msg","OK");
		mv.setViewName("save_result");
		return mv;
	}
